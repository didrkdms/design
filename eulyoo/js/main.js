$(document).ready(function(){
    ...
    const book_swiper = new Swiper('.book .swiper', { /* 팝업을 감싼는 요소의 class명 */
        slidesPerView: 2,/*화면에 나오는 갯수*/ /* li의 넓이 비율로 안함 - css에서 준 넓이대로 함 */
    ...
    /*스크롤을 내리면 header에 fixed 클래스를 추가
      스크롤이 맨 위에 있을때는 fixed 클래스 추가
                조금이라도 내려오면 fixed 클래스 추가
      브라우저가 스크롤 된 정도로 체크하는 값
      $(window).scrollTop()
      맨 위 0
    ...
    */ 
    ...
    /*
        ...
        --> 이전에 오버했던 li를 선택하는 영역X
            모든 li에 있는 on클래스를 다 삭제
            오버한 li에만 다시 on클래스를 추가
        <ul>
